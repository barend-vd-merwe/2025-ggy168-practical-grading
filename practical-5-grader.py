...
if image is not None:
    ...
    strip_1 = st.number_input("Strip 1", min_value = 0, max_value =1)
    strip_2 = st.number_input("Strip 2", min_value = 0, max_value =1)
    strip_3 = st.number_input("Strip 3", min_value = 0, max_value =1)
    strip_4 = st.number_input("Strip 4", min_value = 0, max_value =1)

    
    st.header("Answer")
    st.write('''
        The answer should lie between 28.28km² and 34.56km² (2 marks). The units should
        be shown as well (km²) (2 marks).
    ''')
    answer_img = warped_img[114:114+43, 547:547+164]
    st.image(answer_img)
    answer_val = st.number_input("Answer Value Correct", min_value = 0, max_value = 2)
    answer_units = st.number_input("Answer Units Correct", min_value = 0, max_value = 2)
    ...
    if st.button("Grade"):
        ...
        cv.putText(img=warped_img, text=f'{final_grade}', org=(381,44+37),
